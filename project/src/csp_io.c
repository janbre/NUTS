...
/** Validate SEQNR Socket option */
#ifndef CSP_USE_SEQNR
	if(opts & CSP_SO_SEQNR) {
		csp_log_error("Attempt to create socket that uses sequence numbers, but CSP was compiled withou global SEQNR support\r\n");
		return NULL;
	}
#endif
	...
	if (opts & ~(CSP_SO_RDPREQ | CSP_SO_XTEAREQ | CSP_SO_HMACREQ | CSP_SO_CRC32REQ | CSP_SO_CONN_LESS | CSP_SO_SEQNR)) {
