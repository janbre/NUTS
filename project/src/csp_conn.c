...
/** Sequence numbers **/
	if(opts & CSP_O_SEQNR) {
#ifdef CSP_USE_SEQNR
		outgoing_id.flags |= CSP_SEQNR;
		incoming_id.flags |= CSP_SEQNR;
#else
		csp_log_error("Attempt to create a connection with a sequence number, but CSP was compiled without SEQNR support \r\n");
		return NULL;
#endif
	}	
